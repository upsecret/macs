...
- [ ] **Gateway CORS 정책 좁히기** — 현재 `gateway-service/src/main/java/com/macs/gateway/config/CorsConfig.java` 가 `allowedOriginPatterns="*"`, `allowedMethods="*"`, `allowedHeaders="*"`, `allowCredentials=true` 로 모든 origin/method/header 를 허용한다(PoC 편의). 운영 전 허용 origin 화이트리스트(예: `https://portal.macs.internal`) + 필요한 메서드/헤더만 명시. credentials 를 끄거나 origin 와일드카드 제거 검토.
